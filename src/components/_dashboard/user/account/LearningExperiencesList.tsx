...
export default function LearningExperiencesList({
  ...
  const handleSelectAllClick = (checked: boolean) => {
    if (checked) {
      const newSelecteds = learningExperiencesList.map((n: any) => n.name);
  ...
  const emptyRows = !isLoading && !learningExperiencesList;
  ...
  const isDiverNotFound = learningExperiencesList.length === 0 && isLoading;
              ...
              <TableContainer sx={{ minWidth: 800 }}>
                <Table>
                  <TeacherListHead
                    ...
                    rowCount={learningExperiencesList.length}
